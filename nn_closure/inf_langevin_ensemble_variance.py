"""
Run ensemble inference for the neural Langevin closure.

For each trained `.keras` model, the script rolls out an ensemble for each IC
from `dataset.npz` and saves ensemble variance (and optionally mean) at
log-spaced times.
"""
...
parser.add_argument(
    ...
    type=int,
    default=1_000_000,
    help="Number of rollout steps with noise (0 = no noise, >= num_steps = always on)",
)
...
# -----------------------
u0_ic = u_gt[:, :n_ics, t0]
u0_ic_ens = np.repeat(u0_ic[:, :, None], repeats=n_ens, axis=2)
u0_all = u0_ic_ens.reshape(N, n_ics * n_ens)
batch = n_ics * n_ens
...
def ensemble_var_and_mean(aux_np: np.ndarray, n_ics: int, n_ens: int):
    u3 = aux_np.reshape(N, n_ics, n_ens)
    mu = u3.mean(axis=2)
    var = np.mean(np.abs(u3 - mu[:, :, None])**2, axis=2)
    return var, mu
...
for model_name in keras_files:
    ...
    aux = tf.convert_to_tensor(u0_all, dtype=tf_c_prec)
