"""
Run rollouts of the trained neural Langevin closure on initial conditions from
`dataset.npz` and save snapshots every `save_stride` steps.

The script loads all `.keras` models found in `outputs_langevin_closure/`.
"""
...
k0 = np.array(k_list, dtype=np_c_prec)
ek0 = np.array(ek_list, dtype=np_c_prec)
forcing = np.array(forcing_list, dtype=np_c_prec)
forcing0 = np.concatenate((forcing, np.zeros(N - 2, dtype=np_c_prec)))
k2_0 = np.array(k2_list, dtype=np_c_prec)
...
save_stride = 100
num_steps_eff = (num_steps // save_stride) * save_stride
num_save = num_steps_eff // save_stride
t_idx = np.arange(save_stride, num_steps_eff + 1, save_stride, dtype=np.int64)
...
u_gt = data_gt["u"].astype(np_c_prec)
...
if n_ics % batch_size != 0:
    raise ValueError("n_ics must be divisible by batch_size.")
...
sigma_last2 = (
    ...
    * dt**0.5
    * np.real(k0[-2:])**0.5
    * (np.mean(np.abs(u_last2)**2, axis=(1, 2))**0.5) ** (3 / 2)
)
print(f"std last 2 shells: {sigma_last2}")

sigma_last2 = tf.constant(
    sigma_last2.astype(np.float32 if PRECISION == 32 else np.float64),
    dtype=ilayer,
)
...
def time_evol(model, aux):
    ...
    u11_mu = tf.complex(pred[:, 0, 0], pred[:, 0, 1])
    u12_mu = tf.complex(pred[:, 1, 0], pred[:, 1, 1])
    ...
    corr_10_det = dt * 1j * (
        a * k2[-2, :] * u12_mu * tf.math.conj(u11_mu)
...
for model_name in keras_files:
    ...
    for ib in range(0, n_ics, batch_size):
        ...
        k = tf.convert_to_tensor(np.transpose(np.tile(k0, (bs, 1))), dtype=tf_c_prec)
        ...
        k2 = tf.convert_to_tensor(np.transpose(np.tile(k2_0, (bs, 1))), dtype=tf_c_prec)

        aux = tf.convert_to_tensor(u_gt[:, batch_start:batch_end, 0], dtype=tf_c_prec)
