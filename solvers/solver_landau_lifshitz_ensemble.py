...
parser.add_argument("--ic-index", type=int, default=0, help="IC index in dataset.npz to use")
...
# Load ICs from the last saved time of dataset.npz: u shape (N, n_ics, T)
u_all = np.load("dataset.npz")["u"]
if u_all.ndim != 3 or u_all.shape[0] != N:
    raise ValueError(f"Expected dataset.npz['u'] shape (N, n_ics, T), got {u_all.shape}")

n_ics_in_file = u_all.shape[1]
if ic_index < 0 or ic_index >= n_ics_in_file:
    raise ValueError(f"--ic-index {ic_index} out of range (n_ics={n_ics_in_file})")

# Take one IC at the last saved time and tile across ensemble members: (N, n_ens)
aux0 = jnp.asarray(u_all[:, ic_index, -1], dtype=jnp.complex128)[:, None]
aux = jnp.tile(aux0, (1, n_ens))

batch_size = n_ens
print(f"Running ONE IC (index {ic_index}) with n_ens={n_ens}")
print("aux shape:", aux.shape)

k = jnp.tile(k_1d[:, None], (1, batch_size)).astype(jnp.complex128)
...
kpow = (k_1d ** (1.0 + 0.5 * alpha))[:, None]
sigma_step = (jnp.sqrt(Theta * dt) * kpow).astype(jnp.complex128)
...
def _one_step(carry, _):
    ...
    z = random.normal(k_noise, aux_.shape + (2,), dtype=jnp.float64)
    dW = z[..., 0] + 1j * z[..., 1]
